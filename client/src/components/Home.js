    ...
    return (
      <>
        <div className="w-6/6 mx-5 mt-5">
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-5 border border-slate-400 rounded-md p-0.5">
          ...
          <div className={'grid grid-cols-2 lg:grid-cols-3 gap-4 border border-slate-400 rounded-md p-0.5'}>
            {valves.map((value) => (<ValveChip name={value.name} state={value.status}/>))}
          </div>
        </div>
      </>
    );
